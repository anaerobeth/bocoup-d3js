  ...
  currentCircles
    ...
    // ...?
      ...
      .attr('r', function(country) {
        return radiusScale(country.current[rIndicator]);
      })
      .attr('cx', function(country) {
        return xScale(country.current[xIndicator]);
      })
      .attr('cy', function(country) {
        return yScale(country.current[yIndicator]);
      })
...
function draw() {
  ...
  if (++year <= endYear) {
    setTimeout(draw, updateInterval/2);
  }
}
