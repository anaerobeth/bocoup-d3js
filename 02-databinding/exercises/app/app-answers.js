...
var table = viewer.append('table').classed('countries', true);

// Task 3:
// create a row in the table that has the class name 'headings'
var headings = table.append('tr').classed('headings', true);
...
d3.json("../assets/countrydata.json", function(error, data) {
    ...
    var rows = table.selectAll('tr.foo')
    ...
    newRows.append('td').text(function(d) {
      if (d.population) {
        return formatter(d.population[d.population.length -1].value);
      } else {
        return 'NA'
      }
    });
