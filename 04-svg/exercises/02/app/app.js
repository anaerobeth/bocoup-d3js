...
d3.json("/assets/countrydata.json", function(error, data) {
  ...
  function plotCircles() {
    ...
    var validNations = data.filter(function(country) {
       // ???
       return country.current[rIndicator] && country.current[yIndicator] && country.current[xIndicator];
    });
    ...
    enteringCircles.append("circle")
      ...
      // .attr("cx", function(d) { ? })
      // .attr("cy", function(d) { ? })
      .attr("r", function(d) { radiusScale(d.current[rIndicator]); })
      .attr("cx", function(d) { xScale(d.current[xIndicator]); })
      .attr("cy", function(d) { yScale(d.current[yIndicator]); })
