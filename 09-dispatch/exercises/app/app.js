...
var titleField = viewer.append('div')
    .attr('id', 'country')
    .text(String)
...
function createMap() {
  ...
  d3.json("../assets/countrygeo.json", function(error, world) {
    ...
    dispatch.on('country_select', function(name) {
      map.select("path[name='" + name + "']").style('opacity', 0.5);
      titleField.text(name).style('opacity', 1);
    });

    dispatch.on('country_select_end', function(name) {
      map.select("path[name='" + name + "']").style('opacity', 1);
      titleField.text(name).style('opacity', 0);
    });
  });
}
